<?php
    ...
    function enviaArquivo($professor, $turma, $disciplina, $arquivo){
        ...
        $busca1 = "select * from lembrete where limite_plano = 1";
        ...
        $busca4 = "select * from plano where prof = '{$professor}' and disc = '{$disciplina}' and turma = '{$turma}'";
        $resB4 = $conn->query($busca4) or die($conn->error);
        $qtdB4 = $resB4->num_rows;

        if($qtdB1 > 0){
            $busca2 = "select * from plano where prof = '{$professor}' and disc = '{$disciplina}' and turma = '{$turma}' and dat <= '{$rowB1->dt}'";
            $resB2 = $conn->query($busca2) or die($conn->error);
            $qtdB2 = $resB2->num_rows;

            $busca3 = "select * from plano where prof = '{$professor}' and disc = '{$disciplina}' and turma = '{$turma}' and dat > '{$rowB1->dt}'";
            $resB3 = $conn->query($busca3) or die($conn->error);
            $qtdB3 = $resB3->num_rows;
            if($rowB1->dt > date("Y-m-d")){
                if($qtdB2 == 0){
                    $sql1 = "insert into plano(prof, turma, disc, arquivo, dat) values('{$professor}', '{$turma}', '{$disciplina}', '{$arquivo}', '".date('Y-m-d')."')";
                    $conn->query($sql1) or die($conn->error);
                } else if ($qtdB2 > 0){
                    $rowB2 = $resB2->fetch_object();
                    echo "é no B2 <br>";
                    echo date('Y-m-d')."<br>";
                    $sql2 = "update plano set arquivo = '{$arquivo}', dat = '".date('Y-m-d')."' where id = '{$rowB2->id}'";
                ...
                if($qtdB3 == 0){
                    ...
                    echo "é no B3";
                    $sql2 = "update plano set arquivo = '{$arquivo}', dat = '".date('Y-m-d')."' where id = '{$rowB3->id}'";
                    $conn->query($sql2) or die($conn->error);
                }
            }
        } else{
            if($qtdB4 == 0){
                ...
                echo "é no b4";
                $sql2 = "update plano set arquivo = '{$arquivo}', dat = '".date('Y-m-d')."' where id = '{$rowB4->id}'";
                $conn->query($sql2) or die($conn->error);
            }
        }
...
<head>
    ...
    <title>Envio de plano de trabalho</title>
</head>
<body>
    <form action='plano.php' method='POST' enctype='multipart/form-data'>
