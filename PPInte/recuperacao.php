<?php
    ...
    function enviaArquivo($professor, $turma, $disciplina, $arquivo){
        ...
        if($qtdB1 > 0){
            $busca2 = "select * from recuperacao where professor = '{$professor}' and disciplina = '{$disciplina}' and turma = '{$turma}' and dat <= '{$rowB1->dat}'";
            $resB2 = $conn->query($busca2) or die($conn->error);
            $qtdB2 = $resB2->num_rows;

            $busca3 = "select * from recuperacao where professor = '{$professor}' and disciplina = '{$disciplina}' and turma = '{$turma}' and dat > '{$rowB1->dat}'";
            $resB3 = $conn->query($busca3) or die($conn->error);
            $qtdB3 = $resB3->num_rows;
            if($rowB1->dat > date("Y-m-d")){
                if($qtdB2 == 0){
                    ...
                    $sql2 = "update recuperacao set arquivo = '{$arquivo}' where id = '{$rowB2->id}'";
                    $conn->query($sql2) or die($conn->error);
                }
            } else if($rowB1->dat < date("Y-m-d")){
                if($qtdB3 == 0){
                    $sql1 = "insert into recuperacao(professor, turma, disciplina, arquivo, dat) values('{$professor}', '{$turma}', '{$disciplina}', '{$arquivo}', ".date('Y-m-d').")";
                    $conn->query($sql1) or die($conn->error);
                } else if ($qtdB3 > 0){
                    $rowB3 = $resB3->fetch_object();
                    $sql2 = "update recuperacao set arquivo = '{$arquivo}' where id = '{$rowB3->id}'";
                    $conn->query($sql2) or die($conn->error);
                }
            }
        } else{
